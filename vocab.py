...
vocab.py: Vocabulary Generation, modified from Homework 5
...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.start_of_word]+[self.char2id[w[i]] for i in range(len(w))]+[self.end_of_word] for w in s] for s in sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_padded = pad_sents_char(sents_char, self.char2id['<pad>'])
